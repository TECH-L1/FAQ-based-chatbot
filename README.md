# 🤖 **PDF Document-Based Chatbot with Local LLM**

**TL;DR**: Fully offline PDF document-based chatbot using Ollama + LangChain for local document analysis and intelligent Q&A responses.

---

## 🎯 Overview
This project is an **intelligent chatbot based on PDF documents**. It leverages **LangChain** and **local LLMs via Ollama** to retrieve information from uploaded PDF documents and delivers responses through an intuitive **Streamlit web interface**.

### ✨ Key Features
- **🏠 Local LLM Support** - Run completely offline using Ollama with DeepSeek-R1-Distill-Qwen-14B
- **📄 Multi-Format Document Processing** - Support for PDF, Excel, Word, Text, and CSV files
- **🔍 Intelligent Document Search** - Vector-based similarity search with FAISS
- **💬 Interactive Web UI** - Clean Streamlit interface with conversation history
- **🔧 Flexible Configuration** - Support both local (Ollama) and cloud (OpenAI) LLMs
- **📊 Performance Optimized** - Local embeddings and efficient document processing
...
├── data/
│   ├── documents/             # Your document files go here
...
- Best results with structured content and clear Q&A formatting
...
Example: "How to create project?","Step 1: Login to console...","Getting Started","project,setup"
```
...
1. **Load Document Database**: Click "🔄 Load Document Database" in the sidebar
2. **Ask Questions**: Type document-related questions in the chat input
3. **View Responses**: Get contextual answers based on your document content
...
3. Reload document database in the web interface
